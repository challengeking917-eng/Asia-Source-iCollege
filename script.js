...
var DEFAULT_DB = {
  students: [{
    id:"STU-001", emailUser:"juan.delacruz@gmail.com", pass:"2008-03-15",
    name:"Juan dela Cruz", email:"juan.delacruz@gmail.com", strand:"G11-STEM-A", bday:"2008-03-15", gender:"Male",
    studentId:"24-0561", mobile:"09XX-XXX-XXXX", address:"", city:"",
    grades:{}, scores:{ww:25,pt:24,qe:36,act:18}, approvedSubjects:[]
  }],
  teachers: [{
    id:"TCH-001", emailUser:"ana.santos@gmail.com", pass:"1985-06-12",
    name:"Ms. Ana Santos", email:"ana.santos@gmail.com", bday:"1985-06-12", address:""
  }],
  classRoster: {
...
/* ── FIXED THEME FUNCTIONS ──
   Uses body.login-active class to completely isolate login page
   from dashboard dark/light theme toggling.
   Login page always shows its own brown/warm design.
   Dashboard theme changes never affect the login page.
──────────────────────────────────────────────────────────── */
function toggleDark() {
  // Never allow theme toggle while on login page
...
function applyLoginDark() {
  // Add login-active to body — this scopes all login CSS
  document.body.classList.add("login-active");
  // Keep dark as base so general CSS vars resolve correctly
  document.documentElement.setAttribute("data-theme", "dark");
  isDark = true;
}

function applyUserTheme() {
  // Remove login-active — dashboard now controls the theme
...
function switchAuth(tab) {
  if (tab === "login") { showEl("authLogin"); hideEl("authSignup"); }
  else                 { hideEl("authLogin"); showEl("authSignup"); }
}
...
function setSuRole(role) {
  suRole = role;
  g("suBtnStu").className = "lp-role-link" + (role === "student" ? " active" : "");
  g("suBtnTch").className = "lp-role-link" + (role === "teacher" ? " active" : "");
...
function handleLogin(e) {
  e.preventDefault();
  var fresh = loadDB();
  if (fresh && Array.isArray(fresh.students)) { DB.students = fresh.students; DB.teachers = fresh.teachers; }
  
  var rawInput = g("userId").value.replace(/\s+/g, "").toLowerCase();
  var eu = rawInput;
  ...
  if (stu) {
    currentUser = stu; currentRole = "student";
    hideEl("loginPage"); applyUserTheme();
    loadStudentDash(stu); showEl("studentDash");
    startStudentPolling(); return;
  }
  ...
  if (tch) {
    currentUser = tch; currentRole = "teacher";
    hideEl("loginPage"); applyUserTheme();
    loadTeacherDash(tch); showEl("teacherDash"); return;
  }

  var all = DB.students.concat(DB.teachers);
  showAlert("alertBox", all.some(function(u) { return u.emailUser === eu; })
    ? "Wrong password. Use your birthday: YYYY-MM-DD"
    : "Email not found. Check spelling or sign up first.");
}
/* ── FIXED logout ── */
function logout() {
  stopStudentPolling();
  hideEl("studentDash"); hideEl("teacherDash");
  currentUser = null; currentRole = "student";
  g("userId").value = ""; g("password").value = "";
  showEl("loginPage"); applyLoginDark();
}
...
function loadStudentDash(stu) {
  ...
  g("stuProfileEmail").textContent= stu.email;
...
function selectSubjectFromCatalog(subj) {
  ...
  DB.teachers.forEach(function(t) {
    html += '<div class="asj-teacher-card" id="asjTcard_' + t.id + '" onclick="selectTeacherForSubject(\'' + t.id + '\',\'' + t.emailUser + '\',this)">' +
      '<div class="asj-tcard-avatar">👩‍🏫</div><div class="asj-tcard-info"><div class="asj-tcard-name">' + t.name + '</div><div class="asj-tcard-email">' + t.emailUser + '</div>' +
...
function loadTeacherDash(tch) {
  ...
  g("tchProfileEmail").textContent= tch.email;
...
function openTeacherNotifPanel() {
  ...
  if (!notifs.length) html = '<div class="notif-empty">📭 No notifications yet</div>';
    ...
    notifs.forEach(function(n) {
      ...
      html += '<div class="notif-email">' + n.studentEmail + '</div>';
...
function renderAddStudentList() {
  ...
  DB.students.forEach(function(stu) {
    ...
    html += '<div class="add-stu-card"><div class="astu-avatar">🎓</div><div class="astu-info"><div class="astu-name">' + stu.name +
      ...
      '</div><div class="astu-email">' + stu.emailUser + '</div><div class="astu-strand">' + (getGradeLevel(stu.strand) || "") + (getGradeLevel(stu.strand) && strandLabel ? " · " : "") + (strandLabel || "") + '</div></div>' +
...
function renderGradebook(strand, section) {
  ...
  if (toolbar) {
    toolbar.innerHTML = stripHtml +
      ...
      '</div><button class="save-qtr-btn" onclick="saveQuarterGrades(\'' + key + '\')">💾 Save ' + currentQuarter.toUpperCase() +' </button></div>';
  }
...
function openProfileModal() {
  ...
  g("pmEmailDisplay").textContent = (currentUser.email || currentUser.emailUser);
  ...
  var fields = {
    ...
    pmInfoEmail:     (currentUser.email || currentUser.emailUser),
...
/* ============================================================
   ...
   ============================================================ */
(function init() {
  // ── CRITICAL: Mark body as login-active on page load ──
  // This ensures the login page always shows its brown/warm design
  // regardless of what theme the user had on the dashboard.
  document.body.classList.add("login-active");
...
function _buildMasterStudentList() {
  ...
  (DB.students || []).forEach(function(stu) {
    seen[stu.name] = true;
    list.push({ id:stu.id, name:stu.name, emailUser:stu.emailUser||stu.email||"", strand:stu.strand||"", gender:stu.gender||"", isRegistered:true, bday:stu.bday||"" });
  });
  var allRosterKeys = [
  ...
  allRosterKeys.forEach(function(rk) {
    ...
    ["male","female"].forEach(function(gen) {
      (roster[gen] || []).forEach(function(name) {
        ...
        list.push({ id:null, name:name, emailUser:"", strand:rk, gender:gen==="female"?"Female":"Male", isRegistered:false, bday:"" });
      });
    });
  });
  return list;
...
function _countAllUniqueStudents() { return _buildMasterStudentList().length; }
...
function renderStudentManagerModal() {
  ...
  var regCount = (DB.students||[]).length, rosterCount = total - regCount;
  var html = '<div class="stm-wrap">';
  html += '<div class="stm-header"><div class="stm-header-icon">🎓</div><div class="stm-header-text"><h2 class="stm-title">Student Manager</h2><p class="stm-sub">Edit info, transfer strand/section, or remove students</p></div><div class="stm-counter"><span class="stm-counter-num" id="stmCounterNum">' + total + '</span><span class="stm-counter-lbl">students</span></div></div>';
  html += '<div class="stm-search-row"><div class="stm-search-box"><span class="stm-search-ico">🔍</span><input class="stm-search-input" type="text" id="stmSearch" placeholder="Search by name, email, or strand..." oninput="filterStudentManager()"><button class="stm-search-clear hidden" id="stmClear" onclick="clearStmSearch()">✕</button></div></div>';
  html += '<div style="display:flex;gap:0;padding:10px 20px 8px;background:rgba(0,0,0,0.2);border-bottom:1px solid rgba(255,255,255,0.06);">' +
    '<div style="flex:1;text-align:center;border-right:1px solid rgba(255,255,255,0.08);"><div style="font-family:\'Cinzel\',serif;font-size:20px;font-weight:900;color:var(--gold);">' + total + '</div><div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--muted);">Total</div></div>' +
    '<div style="flex:1;text-align:center;border-right:1px solid rgba(255,255,255,0.08);"><div style="font-family:\'Cinzel\',serif;font-size:20px;font-weight:900;color:#5dde92;">' + regCount + '</div><div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--muted);">Registered</div></div>' +
    '<div style="flex:1;text-align:center;"><div style="font-family:\'Cinzel\',serif;font-size:20px;font-weight:900;color:var(--gold);">' + rosterCount + '</div><div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--muted);">Roster Only</div></div></div>';
...
var STM_STRAND_OPTS = [
  ...
  ["G11-HE-HO-A","G11 | H.E-H.O | Section A"],["G11-HE-HO-B","G11 | H.E-H.O | Section B"],
  ...
  ["G12-HE-HO-A","G12 | H.E-H.O | Section A"],["G12-HE-HO-B","G12 | H.E-H.O | Section B"],
...
function _strandSelectHtml(inputId, currentStrand) {
  ...
  var g11 = STM_STRAND_OPTS.filter(function(o){return o[0].indexOf("G11")===0;});
  var g12 = STM_STRAND_OPTS.filter(function(o){return o[0].indexOf("G12")===0;});
  html += '<optgroup label="── GRADE 11 ──">';
  g11.forEach(function(o){html+='<option value="'+o[0]+'"'+(currentStrand===o[0]?' selected':'')+'>'+o[1]+'</option>';});
  html += '</optgroup><optgroup label="── GRADE 12 ──">';
  g12.forEach(function(o){html+='<option value="'+o[0]+'"'+(currentStrand===o[0]?' selected':'')+'>'+o[1]+'</option>';});
...
function _buildStmListHtml(query) {
  ...
  if (q) { all = all.filter(function(stu){ return (stu.name||"").toLowerCase().indexOf(q)>=0||(stu.emailUser||"").toLowerCase().indexOf(q)>=0||(stu.strand||"").toLowerCase().indexOf(q)>=0; }); }
  if (!all.length) return '<div class="stm-empty"><span class="stm-empty-icon">🔍</span><div>No students found' + (q ? ' for "' + query + '"' : '') + '</div></div>';
  all.sort(function(a,b){ var sa=a.strand||"ZZZ",sb=b.strand||"ZZZ"; if(sa<sb)return -1;if(sa>sb)return 1;return(a.name||"").localeCompare(b.name||""); });
  var grouped={}, groupOrder=[];
  all.forEach(function(stu){ var key=stu.strand||"Unknown"; if(!grouped[key]){grouped[key]=[];groupOrder.push(key);}grouped[key].push(stu); });
  var html="";
  groupOrder.forEach(function(strandKey){
    var students=grouped[strandKey];
    var glShort=getGradeLevelShort(strandKey), sn=getStrandName(strandKey), sec=getSectionLetter(strandKey);
    var glColor=(strandKey.indexOf("G11")===0)?"#0ea5e9":"#10b981";
    var fullLabel=DB.strandFull[strandKey]||(glShort+" — "+sn+(sec?" / Section "+sec:""));
    html+='<div style="display:flex;align-items:center;gap:8px;padding:12px 4px 5px;margin-top:4px;">' +
      '<span style="background:'+glColor+'22;border:1px solid '+glColor+'55;color:'+glColor+';border-radius:20px;padding:3px 14px;font-size:10px;font-weight:800;letter-spacing:.5px;white-space:nowrap;">'+glShort+'</span>' +
      '<span style="font-size:12px;font-weight:800;color:var(--text);white-space:nowrap;">'+(sn||strandKey)+(sec?' — Section '+sec:'')+'</span>' +
      '<div style="flex:1;height:1px;background:rgba(255,255,255,0.07);margin:0 4px;"></div>' +
      '<span style="font-size:10px;color:var(--muted);white-space:nowrap;">'+students.length+' student'+(students.length!==1?'s':'')+'</span></div>';
    students.forEach(function(stu,i){
      var nameHtml=stu.name;
      if(q){var ni=stu.name.toLowerCase().indexOf(q);if(ni>=0)nameHtml=stu.name.slice(0,ni)+'<mark class="stm-hl">'+stu.name.slice(ni,ni+q.length)+'</mark>'+stu.name.slice(ni+q.length);}
      var statusBadge=stu.isRegistered?'<span style="background:rgba(0,230,118,0.12);border:1px solid rgba(0,230,118,0.3);color:#5dde92;border-radius:20px;padding:2px 8px;font-size:9px;font-weight:800;">✓ Registered</span>':'<span style="background:rgba(255,184,0,0.1);border:1px solid rgba(255,184,0,0.3);color:var(--gold);border-radius:20px;padding:2px 8px;font-size:9px;font-weight:800;">Roster Only</span>';
      var genderIcon=(stu.gender&&stu.gender.toLowerCase()==="female")?"👩":"🎓";
      var gradePill='<span style="background:'+glColor+'22;border:1px solid '+glColor+'55;color:'+glColor+';border-radius:6px;padding:2px 9px;font-size:9.5px;font-weight:800;white-space:nowrap;">'+glShort+(sec?' · Sec '+sec:'')+'</span>';
      var safeId=stu.id?stu.id:("_r_"+strandKey+"_"+i);
      var cardId="stmCard_"+safeId;
      var actionBtns='<button class="stm-btn stm-btn-edit" onclick="openEditStudentInline(\''+safeId+'\',\''+stu.name.replace(/'/g,"\\'")+'\',\''+stu.strand+'\',\''+(stu.gender||'')+'\')">✏️ Edit</button>' +
        (stu.id?'<button class="stm-btn stm-btn-delete" onclick="confirmDeleteStudent(\''+stu.id+'\')">🗑️ Remove</button>':'<button class="stm-btn stm-btn-delete" onclick="confirmRemoveRosterStudent(\''+stu.name.replace(/'/g,"\\'")+'\',\''+stu.strand+'\')">🗑️ Remove</button>');
      html+='<div class="stm-card" id="'+cardId+'"><div class="stm-card-avatar">'+genderIcon+'</div><div class="stm-card-info"><div class="stm-card-name">'+nameHtml+' '+gradePill+' '+statusBadge+'</div>'+(stu.emailUser?'<div class="stm-card-email">'+stu.emailUser+'</div>':'<div class="stm-card-email" style="color:rgba(255,255,255,0.2);font-style:italic;">No account yet</div>')+'<div class="stm-card-strand">'+fullLabel+'</div></div><div class="stm-card-actions">'+actionBtns+'</div></div>';
    });
  });
  return html;
}
...
function _refreshStmList() {
  var listEl=document.getElementById("stmList"); if(listEl)listEl.innerHTML=_buildStmListHtml(_stuMgrSearch);
  var ctr=document.getElementById("stmCounterNum"); if(ctr)ctr.textContent=_countAllUniqueStudents();
}
function filterStudentManager() {
  var inp=document.getElementById("stmSearch"); _stuMgrSearch=inp?inp.value:"";
  var clr=document.getElementById("stmClear"); if(clr)clr.classList.toggle("hidden",!_stuMgrSearch);
  _refreshStmList();
}
...
function openEditStudentInline(safeId, currentName, currentStrand, currentGender) {
  var cardId="stmCard_"+safeId, card=document.getElementById(cardId); if(!card)return;
  var strandSel=_strandSelectHtml("stmEditStrand_"+safeId, currentStrand);
  card.innerHTML='<div class="stm-edit-form" style="width:100%;">' +
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">' +
    '<div class="stm-edit-row"><label class="stm-edit-lbl">Full Name</label><input class="stm-edit-input" type="text" id="stmEditName_'+safeId+'" value="'+(currentName||"").replace(/"/g,"&quot;")+'" placeholder="Student full name"></div>' +
    '<div class="stm-edit-row"><label class="stm-edit-lbl">Gender</label><select class="stm-edit-select" id="stmEditGender_'+safeId+'"><option value=""'+(!currentGender?' selected':'')+'>— Select —</option><option value="Male"'+(currentGender==='Male'?' selected':'')+'>Male</option><option value="Female"'+(currentGender==='Female'?' selected':'')+'>Female</option></select></div>' +
    '</div>' +
    '<div class="stm-edit-row"><label class="stm-edit-lbl">🎓 Grade, Strand &amp; Section</label>'+strandSel+'</div>' +
    '<div id="stmTransferNotice_'+safeId+'" style="display:none;background:rgba(255,184,0,0.08);border:1px solid rgba(255,184,0,0.3);border-radius:10px;padding:8px 12px;font-size:11px;color:var(--gold);">⚡ Changing strand will move this student to the new class roster.</div>' +
    '<div class="stm-edit-actions"><button class="stm-btn stm-btn-save" onclick="saveEditStudentUniversal(\''+safeId+'\',\''+( currentName||"").replace(/'/g,"\\'")+'\',\''+currentStrand+'\')">💾 Save Changes</button><button class="stm-btn stm-btn-cancel" onclick="_refreshStmList()">✕ Cancel</button></div></div>';
  var selEl=document.getElementById("stmEditStrand_"+safeId);
  if(selEl){selEl.addEventListener("change",function(){var notice=document.getElementById("stmTransferNotice_"+safeId);if(notice)notice.style.display=(this.value!==currentStrand)?"":"none";});}
}
...
function saveEditStudentUniversal(safeId, oldName, oldStrand) {
  ...
  var isRegistered=safeId.indexOf("_r_")!==0, changed=[];
  if(isRegistered){
    for(var i=0;i<DB.students.length;i++){if(DB.students[i].id===safeId){if(DB.students[i].name!==newName)changed.push("name");if(DB.students[i].strand!==newStrand)changed.push("strand → "+(DB.strandFull[newStrand]||newStrand));DB.students[i].name=newName;DB.students[i].strand=newStrand;if(newGender)DB.students[i].gender=newGender;break;}}
    ...
    if(oldName!==newName)_renameStudentInRosters(oldName,newName);
    if(oldStrand!==newStrand){_removeNameFromRoster(oldName,oldStrand);_addNameToRoster(newName,newStrand,newGender);changed.push("strand → "+(DB.strandFull[newStrand]||newStrand));}
    else if(oldName!==newName){_removeNameFromRoster(oldName,oldStrand);_addNameToRoster(newName,newStrand,newGender||"male");}
    if(oldName!==newName)changed.push("name");
  }
  saveDB();saveGrades();
  var msg='✅ '+newName+' updated!';if(changed.length)msg+=' ('+changed.join(', ')+')';
  showToast(msg,'success');_refreshStmList();
  if(typeof renderGradebook==='function')renderGradebook(currentFolder,currentSection);
...
function _removeNameFromRoster(name,strand){
  ...
  keys.forEach(function(rk){if(!DB.classRoster[rk])return;["male","female"].forEach(function(gg){DB.classRoster[rk][gg]=(DB.classRoster[rk][gg]||[]).filter(function(n){return n!==name;});});});
}
function _addNameToRoster(name,strand,gender){
  var rk=strand;if(!DB.classRoster[rk])DB.classRoster[rk]={male:[],female:[]};
  var gg=(gender&&gender.toLowerCase()==="female")?"female":"male";
  if(DB.classRoster[rk][gg].indexOf(name)<0)DB.classRoster[rk][gg].push(name);
}
function _renameStudentInRosters(oldName,newName){
  for(var rk in DB.classRoster){["male","female"].forEach(function(gen){var arr=DB.classRoster[rk][gen]||[];for(var i=0;i<arr.length;i++){if(arr[i]===oldName){arr[i]=newName;break;}}});}
  for(var ck in rosterGrades){if(rosterGrades[ck][oldName]!==undefined){rosterGrades[ck][newName]=rosterGrades[ck][oldName];delete rosterGrades[ck][oldName];}}
}
function _transferStudentStrand(stu,oldStrand,newStrand,newName,gender){
  _removeNameFromRoster(stu.name,oldStrand);if(stu.name!==newName)_removeNameFromRoster(newName,oldStrand);
...
function confirmDeleteStudent(stuId){
  ...
  card.innerHTML='<div class="stm-confirm-delete"><div class="stm-confirm-icon">⚠️</div><div class="stm-confirm-text"><strong>Remove '+stu.name+'?</strong><span>This removes their account and grade records. This cannot be undone.</span></div><div class="stm-confirm-actions"><button class="stm-btn stm-btn-danger" onclick="deleteStudent(\''+stuId+'\')">🗑️ Yes, Remove</button><button class="stm-btn stm-btn-cancel" onclick="_refreshStmList()">✕ Cancel</button></div></div>';
}
function deleteStudent(stuId){
  ...
  for(var rk in DB.classRoster){["male","female"].forEach(function(gg){DB.classRoster[rk][gg]=(DB.classRoster[rk][gg]||[]).filter(function(n){return n!==stuName;});});}
  ...
  saveDB();saveGrades();
  showToast('🗑️ '+stuName+' removed.','warning');_refreshStmList();
  if(typeof renderGradebook==='function')renderGradebook(currentFolder,currentSection);
  if(typeof renderClassRecord==='function')renderClassRecord(currentFolder,currentSection);
}
function confirmRemoveRosterStudent(stuName,strand){
  stuName=String(stuName).trim();
  var allCards=document.querySelectorAll('.stm-card'),targetCard=null;
  allCards.forEach(function(card){if(!card.id||card.id.indexOf('_r_')<0)return;var nameEl=card.querySelector('.stm-card-name');if(nameEl&&nameEl.textContent&&nameEl.textContent.indexOf(stuName)>=0){targetCard=card;}});
  if(!targetCard)return;
  targetCard.innerHTML='<div class="stm-confirm-delete"><div class="stm-confirm-icon">⚠️</div><div class="stm-confirm-text"><strong>Remove '+stuName+'?</strong><span>This removes them from the class roster. This cannot be undone.</span></div><div class="stm-confirm-actions"><button class="stm-btn stm-btn-danger" onclick="deleteRosterOnlyStudent(\''+stuName.replace(/'/g,"\\'")+'\',\''+strand+'\')">🗑️ Yes, Remove</button><button class="stm-btn stm-btn-cancel" onclick="_refreshStmList()">✕ Cancel</button></div></div>';
}
function deleteRosterOnlyStudent(stuName,strand){
  stuName=String(stuName).trim();
  var keys=[strand,strand.replace('HE-HO','HEHO'),strand.replace('HEHO','HE-HO')];
  keys.forEach(function(rk){if(!DB.classRoster[rk])return;['male','female'].forEach(function(gg){DB.classRoster[rk][gg]=(DB.classRoster[rk][gg]||[]).filter(function(n){return n!==stuName;});});});
  for(var ck in rosterGrades){if(rosterGrades[ck][stuName])delete rosterGrades[ck][stuName];}
  saveDB();saveGrades();
  showToast('🗑️ '+stuName+' removed from roster.','warning');_refreshStmList();
  if(typeof renderGradebook==='function')renderGradebook(currentFolder,currentSection);
  if(typeof renderClassRecord==='function')renderClassRecord(currentFolder,currentSection);
}
