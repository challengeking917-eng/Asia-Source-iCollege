...
var DEFAULT_DB = {
  ...
  teachers: [{
    id:"TCH-001", emailUser:"aana.santos@gmail.com", pass:"1985-06-12",
...
function handleLogin(e) {
  ...
  var rawInput = g("userId").value.trim().toLowerCase();
  var eu = rawInput.trim().toLowerCase();
...
function loadStudentDash(stu) {
  ...
  g("stuProfileEmail").textContent= stu.email + "@gmail.com";
...
function _buildStmListHtml(query) {
  ...
  groupOrder.forEach(function(strandKey){
    ...
    students.forEach(function(stu,i){
      ...
      html+='<div class="stm-card" id="'+cardId+'"><div class="stm-card-avatar">'+genderIcon+'</div><div class="stm-card-info"><div class="stm-card-name">'+nameHtml+' '+gradePill+' '+statusBadge+'</div>'+(stu.emailUser?'<div class="stm-card-email">'+stu.emailUser+'@asiasourceicollege.edu.ph</div>':'<div class="stm-card-email" style="color:rgba(255,255,255,0.2);font-style:italic;">No account yet</div>')+'<div class="stm-card-strand">'+fullLabel+'</div></div><div class="stm-card-actions">'+actionBtns+'</div></div>';
    });
  });
  return html;
}
